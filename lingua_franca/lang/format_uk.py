...
def nice_number_uk(number, speech=True, denominators=range(1, 21)):
    ...
    if den == 3:
        return return_string[:-1] + 'іх'
    elif den >= 4:
        return return_string[:-1] + 'их'
    ...
    if scientific:
        ...
        if power != 0:
            if ordinals:
                ...
                power_number = pronounce_number_uk(abs(power), places, short_scale, False, ordinals=True)
                power_number = power_number[:-2] + "ому"
                return '{}{} на десять в {}{} степені'.format(
...
def nice_time_uk(dt, speech=True, use_24hour=True, use_ampm=False):
    ...
    if use_24hour:
        ...
        if string[0] == '0':
            speak += pronounce_hour_uk(int(string[1]))
        else:
        ...
        if string[3:5] == '00':
            ...
            if string[3] == '0':
                ...
                speak += pronounce_number_feminine_uk(int(string[4]))
            else:
                speak += pronounce_number_feminine_uk(int(string[3:5]))
        return speak
    else:
        ...
        if dt.minute == 15:
            speak = "чверть на " + pronounce_hour_uk(next_hour, genitive_case=True)
        elif dt.minute == 30:
            speak = "пів на " + pronounce_hour_uk(next_hour, genitive_case=True)
        elif dt.minute == 45:
            speak = "за чверть " + pronounce_hour_uk(next_hour)
        else:
            ...
            if dt.minute == 0:
                ...
                if dt.minute < 10:
                    speak += " нуль"
                speak += " " + pronounce_number_feminine_uk(dt.minute)
...
def pronounce_hour_uk(num, genitive_case=False):
    pronounced = pronounce_number_uk(num, ordinals=True)

    if num == 0:
        return "нульова"
    if num == 3 or num == 23:
        return pronounced[:-2] + ("ю" if genitive_case else "я")
    return pronounced[:-2] + ("у" if genitive_case else "а")
