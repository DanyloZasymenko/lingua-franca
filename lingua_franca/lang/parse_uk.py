...
def generate_plurals_uk(originals):
    ...
    """
    suffixes = ["а", "ах", "ам", "ами", "ні", "ний", "на", "ів", "ам", "и"]
...
_FRACTION_MARKER = {"і", "з", " "}

# decimal marker ( 1 point 5 = 1 + 0.5)
_DECIMAL_MARKER = {"ціла", "цілих", "крапка", "кома"}
...
_WORDS_NEXT_UK = [
    "майбутня", "майбутнє", "майбутньої", "майбутній", "майбутнім", "майбутню",
    "нова", "нове", "новою", "новий", "новим",
    "наступна", "наступне", "наступною", "наступному", "наступний", "наступну", "наступної"
]
_WORDS_PREV_UK = [
    "попередня", "попередньому", "попередній", "попереднім", "попередню",
    "минула", "минулою", "минулому", "минулу", "минулий", "минулим",
    "тому", "той",
]
_WORDS_CURRENT_UK = [
    "дана", "дане", "даному", "даний",
    "нинішня", "нинішнє", "нинішньою", "нинішнім", "нинішній",
    "поточна", "поточне", "поточній", "поточному", "поточний",
    "це", "цим", "цією", "цьому", "цей", "цю",
]
_WORDS_NOW_UK = [
    "тепер",
    "зараз",
]
_WORDS_MORNING_UK = ["ранок", "зранку"]
_WORDS_DAY_UK = ["вдень"]
_WORDS_EVENING_UK = ["вечір", "ввечері", "вечора"]
...
def _extract_whole_number_with_text_uk(tokens, short_scale, ordinals):
    ...
    for idx, token in enumerate(tokens):
        ...
        # In Ukrainian (?) we do no use suffix (1st,2nd,..) but use point instead (1.,2.,..)
        ...
        # Normalize Ukrainian inflection of numbers (один, одна, одне,...)
        if not ordinals:
...
def extract_duration_uk(text):
    ...
    pattern = r"(?P<value>\d+(?:\.?\d+)?)(?:\s+|\-){unit}(?:і|а|и|у|ин|ину)?"
...
def extract_datetime_uk(text, anchor_date=None, default_time=None):
    ...
    def clean_string(s):
        ...
        s = s.lower().replace('?', '').replace('.', '').replace(',', '') \
            .replace("сьогодні ввечері", "ввечері") \
            .replace("сьогодні вночі", "вночі")
        word_list = s.split()
    ...
    markers = ['на', 'в', 'у', 'до', 'це', 'о', 'об',
               'біля', 'цей', 'через', 'за', 'той']
    days = _DAYS_UK
    months = _MONTHS_UK
    recur_markers = days + ['вихідні', 'вікенд']
    months_short = ['січ', 'лют', 'бер', 'кві', 'тра', 'чер', 'лип', 'сер',
                    'вер', 'жов', 'лис', 'гру']
    ...
    for idx, word in enumerate(words):
        ...
        # parse today, tomorrow, day after tomorrow
            ...
            if word_prev and word_prev[0].isdigit():
                ...
                start -= 1
                used = 2
        elif word in ["день", "дня"] and is_numeric(word_prev) and word_next == "тому":
            if word_prev and word_prev[0].isdigit():
                day_offset += -int(word_prev)
                start -= 1
                used = 3
        ...
        for followup in _WORDS_NOW_UK:
            valid_followups.append(followup)
        if (word in ["до", "по", "від", "з", "зі"]) and word_next in valid_followups:
            used = 2
            from_flag = True
            if word_next == "завтра":
                day_offset += 1
    ...
    for idx, word in enumerate(words):
        ...
        # parse half an hour, quarter hour
            ...
            if word_prev in ["пів", "половина"]:
                min_offset = 30
            elif word_prev in ["чверть", "четвертина"]:
                min_offset = 15
            elif word_prev == "через":
                hr_offset = 1
            else:
            ...
            if ':' in word:
                ...
                if remainder == "":
                    ...
                    if next_word in ["am", "pm", "ночі", "ранку", "дня", "вечора"]:
                        remainder = next_word
                        used += 1
                    elif next_word == "години" and word_next_next in ["am", "pm", "ночі", "ранку", "дня", "вечора"]:
                        ...
                        used = 2
                        day_specified = True
                    elif word_next in ["в", "о"] and word_next_next in _WORDS_NIGHT_UK:
                ...
                if (
                    ...
                    if int(str_num) > 100:
                        ...
                        if (word_next_next == "година" or
                            ...
                            (
                                word_next in ["в", "о", "на", "об"] and word_next_next == time_qualifier
                        ...
                        if word_next == "година":
                            used += 1
                        if word_next in ["в", "о", "на", "об"] or word_next_next in ["в", "о", "на", "об"]:
                            used += (1 if word_next in ["в", "о", "на", "об"] else 2)
                            word_next_next_next = words[idx + 3] \
                                if idx + 3 < len(words) else ""
...
def is_fractional_uk(input_str, short_scale=True):
    ...
    """
    if input_str[-2:] == "ті":  # leading number is bigger than one (дві четверті, три п\'ятих)
        input_str = input_str[:-2] + "та"
    elif input_str[-3:] == "тих":
        input_str = input_str[:-3] + "та"
    elif input_str[-4:] == "тина":
        input_str = input_str[:-4] + "та"
    fractions = {"ціла": 1}  # first four numbers have little different format

    for num in _FRACTION_STRING_UK:  # Numbers from 2 to 1 hundred, more is not usually used in common speech
        if num > 1:
            fractions[_FRACTION_STRING_UK[num]] = num
...
def _text_uk_inflection_normalize(word, arg):
    ...
    """
    if word in ["тисяч", "тисячі"]:
        return "тисяча"

    if arg == 1:  # _extract_whole_number_with_text_uk
        if word in ["одна", "одним", "одне", "одною"]:
            return "один"
        if word == "дві":
            return "два"
        if word == "пару":
        ...
        if word in ["години", "годин", "годину", "годині", "годинами"]:
            return "година"
        if word in ["хвилин", "хвилинам", "хвилинами", "хвилину", "хвилини"]:
            return "хвилина"
        if word in ["секунд", "секундам", "секундами", "секунду", "секунди"]:
            return "секунда"
        if word in ["днів", "дні"]:
            return "день"
        if word in ["тижню", "тижні", "тижня", "тижнів"]:
            return "тиждень"
        if word in ["місяця", "місяців", "місяці"]:
            return "місяць"
        if word in ["року", "років", "роки"]:
            return "рік"
        if word in _WORDS_MORNING_UK:
            return "зранку"
        if word in ["полудні", "полудня", "півдні", "опівдні", "півдня", "полудень"]:
            return "південь"
        if word in _WORDS_EVENING_UK:
            return "ввечері"
        if word in _WORDS_NIGHT_UK:
            return "ніч"
        if word in ["вікенд", "вихідним", "вихідних"]:
            return "выхідні"
        if word in ["позавчора", "післявчора"]:
            return "позавчора"
        if word in ["століть", "вік"]:
            return "століття"
        if word in ["десятиліть"]:
            return "десятиліття"

        # Week days
        if word in ["середу", "середи"]:
            return "середа"
        if word in ["четверга", "четвергу"]:
            return "четвер"
        if word in ["п\'ятницю", "п\'ятниці"]:
            return "п\'ятниця"
        if word in ["суботу", "суботи"]:
            return "субота"
        if word in ["неділі", "неділю"]:
            return "неділя"
        if word[-2:] == "ка":
            tmp = word[:-2] + "ок"
            for name in _DAYS_UK:
                if name == tmp:
                    return name

        # Months
        if word in ["листопаді", "листопада"]:
            return "листопад"
        if word in ["лютого", "лютому"]:
            return "лютий"
